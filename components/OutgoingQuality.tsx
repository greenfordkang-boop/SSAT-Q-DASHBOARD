...
import { OutgoingMetric } from '../types';

interface OutgoingQualityProps {
  metrics: OutgoingMetric[];
  onSaveMetric: (payload: OutgoingMetric | OutgoingMetric[]) => Promise<boolean>;
}

const OutgoingQuality: React.FC<OutgoingQualityProps> = ({ metrics, onSaveMetric }) => {
  ...
  // 실적 입력을 위한 상태
  const [entryForm, setEntryForm] = useState<OutgoingMetric>({
  ...
  const handleAnnualTargetSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (window.confirm(`${targetYear}년 모든 달 목표를 ${annualTarget} PPM으로 변경하시겠습니까?`)) {
      setIsSaving(true);
      const batchPayload: OutgoingMetric[] = Array.from({ length: 12 }, (_, i) => {
        const m = i + 1;
      ...
      if (success) {
        ...
        alert(`${targetYear}년 출하품질 목표가 전산에 일괄 등록되었습니다.`);
      } else {
        alert('저장에 실패했습니다. 다시 시도해주세요.');
      }
    }
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm lg:col-span-2">
          <h3 className="text-sm font-black text-slate-800 mb-6 flex justify-between items-center">
            <span className="flex items-center gap-2"><div className="w-1.5 h-4 bg-blue-600 rounded-full"></div> {selectedYear}년 출하품질 PPM 추이</span>
      ...
      <div className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden">
        <div className="p-5 border-b border-slate-100 bg-slate-50 flex items-center justify-between">
          <span className="font-black text-sm text-slate-800 uppercase tracking-tight">출하품질 {selectedYear}년 실적 전산 상세</span>
      ...
      {showEntryModal && (
        <div className="fixed inset-0 z-[1000] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden animate-in zoom-in duration-200">
            <div className="p-6 bg-slate-900 text-white flex justify-between items-center">
               <h3 className="font-black">{entryForm.year}년 {entryForm.month}월 출하품질 실적 전산 등록</h3>
            ...
            <form onSubmit={handleEntrySubmit} className="p-8 space-y-6">
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">대상 연도</label>
                  <input readOnly value={entryForm.year} className="w-full bg-slate-100 border border-slate-200 p-3 rounded-xl font-bold outline-none" />
                </div>
                <div className="space-y-1.5">
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      const existing = metrics.find(item => Number(item.year) === selectedYear && Number(item.month) === m);
                      setEntryForm({
                        ...entryForm,
                        month: m,
