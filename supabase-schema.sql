...
CREATE INDEX IF NOT EXISTS idx_pq_data_part_code ON process_quality_data(part_code);
...
-- 9. Process Defect Type Upload History 테이블 (공정불량유형 업로드 이력)
CREATE TABLE IF NOT EXISTS process_defect_type_uploads (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  filename TEXT NOT NULL,
  record_count INTEGER NOT NULL DEFAULT 0,
  upload_date TIMESTAMPTZ DEFAULT NOW(),
...
-- Process Defect Type Upload History 인덱스
CREATE INDEX IF NOT EXISTS idx_pdt_uploads_date ON process_defect_type_uploads(upload_date DESC);

-- Process Defect Type Uploads RLS 비활성화
ALTER TABLE process_defect_type_uploads DISABLE ROW LEVEL SECURITY;

-- 10. Process Defect Type Data 테이블 (공정불량유형 상세 데이터)
CREATE TABLE IF NOT EXISTS process_defect_type_data (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  upload_id UUID REFERENCES process_defect_type_uploads(id) ON DELETE CASCADE,
  customer TEXT,
  part_code TEXT,
  part_name TEXT,
  process TEXT,
...
-- Process Defect Type Data 인덱스
CREATE INDEX IF NOT EXISTS idx_pdt_data_upload ON process_defect_type_data(upload_id);
CREATE INDEX IF NOT EXISTS idx_pdt_data_customer ON process_defect_type_data(customer);
CREATE INDEX IF NOT EXISTS idx_pdt_data_process ON process_defect_type_data(process);
CREATE INDEX IF NOT EXISTS idx_pdt_data_part_code ON process_defect_type_data(part_code);
CREATE INDEX IF NOT EXISTS idx_pdt_data_date ON process_defect_type_data(data_date DESC);

-- Process Defect Type Data 업데이트 트리거
DROP TRIGGER IF EXISTS update_pdt_data_updated_at ON process_defect_type_data;
CREATE TRIGGER update_pdt_data_updated_at
  BEFORE INSERT OR UPDATE ON process_defect_type_data
  FOR EACH ROW
  EXECUTE FUNCTION update_updated_at_column();
